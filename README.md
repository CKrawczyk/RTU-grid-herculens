...
`std` stand for "standardized coordinates" in this instance and is a reference to a "mass-sheet-like transform invariant" system.  In these coordinates any mass models that are the same up to a sift and uniform scaling of the source grid will resolve to the same coordinates.
...
As the pixels are non-square both the edges and centers are provided.
...
Assuming your observed data is read in as `data`, the PSF kernel is `psf_kernel` and you have a set of mass, light, and source parameters as `mass_params`, `lens_light`, and `source_light` the `model` method can be called as:

```python
PSF_class = SimpleFFTConvolve(psf_kernel, data.shape)

model_image, (grid_centers, grid_edges) = lens_image_pixelated.model(
